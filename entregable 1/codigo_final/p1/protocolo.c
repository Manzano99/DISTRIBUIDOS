...
int sendStringTCP(int s, char *str)
{
    int enviados;
...
int sendStringUDP(int s, char *str, struct sockaddr_in *dir)
{
    int enviados;
    enviados = sendto(s, str, strlen(str), 0, (struct sockaddr *)dir, sizeof(struct sockaddr_in));
    if (enviados != strlen(str))
    {
...
int recibirStringTCP(int s, char *str, int len)
{
    ...
    {
        // A RELLENAR
        str[recibidos] = '\0'; // Añade el carácter nulo al final para terminar
    }
    return recibidos;
}
