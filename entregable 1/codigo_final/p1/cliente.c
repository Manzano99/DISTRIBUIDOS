...
char *enviar_peticion(char *peticion, struct sockaddr_in *d_serv, int es_stream)
{
    ...
    */
    ...
    if (es_stream)
    {
        ...
        sendStringTCP(sock_dat, peticion);
        recibirStringTCP(sock_dat, respuesta, TAMLINEA);
    }
    else // se envía la petición por socket UDP
    {
        // A RELLENAR
        sock_dat = CrearSocketDatosUDP();
        sendStringUDP(sock_dat, peticion, d_serv);
        recibirStringUDP(sock_dat, respuesta, TAMLINEA, d_serv);
    }
    close(sock_dat);
...
void *hilo_descarga(struct sockaddr_in *p)
{
    ...
    while (fgets(buffer, TAMLINEA, fppet) != NULL)
    {
        ...
        // A RELLENAR
        sprintf(peticion, "ISFILE %s", nomfile);
        ...
        if (strcmp(s, "OK") == 0)
        {
            ...
            // A RELLENAR
            sprintf(peticion, "ISREADABLE %s", nomfile);
            s = enviar_peticion(peticion, p, es_stream);
            if (strcmp(s, "OK") == 0)
            {
                free(s);
                ...
                // A RELLENAR
                sprintf(peticion, "GETNUMLINES %s", nomfile);
                s = enviar_peticion(peticion, p, es_stream);
                if (s != NULL)
                {
                    ...
                    if (numlineas > 0)
                    {
                        ...
                        for (i = 0; i < numlineas; i++)
                        {
                            ...
                            // A RELLENAR
                            sprintf(peticion, "GETLINE %d %s", i, nomfile);
                            s = enviar_peticion(peticion, p, es_stream);
...
void main(int argc, char *argv[])
{
    ...
    // A RELLENAR
    th = (pthread_t *)malloc(sizeof(pthread_t) * nhilos);
    if (th == NULL)
    {
    ...
    for (i = 0; i < nhilos; i++)
    {
        if (pthread_create(&th[i], NULL, (void *)hilo_descarga, (void *)&d_serv) != 0)
        {
