...
void *procesarDescargas(datos_hilo *p)
{
    ...
    while (fgets(buffer, TAMLINEA, fppet) != NULL)
    {
        ...
        if (strcmp(token, "T") == 0)
        {
            ...
            // A RELLENAR
            cl = clnt_create(ip_srv, SRVFTP, PRIMERA, "tcp");
            if (cl == NULL) clnt_pcreateerror("Error en TCP"); 
        }
        else if (strcmp(token, "U") == 0)
        {
            fprintf(stderr, "Cliente: %d La descarga del fichero %s se realizará mediante protocolo UDP\n", id_cliente, nomfile);
            // A RELLENAR
            cl = clnt_create(ip_srv, SRVFTP, PRIMERA, "udp");
            if (cl == NULL) clnt_pcreateerror("Error en UDP");
        }
        ...
        // A RELLENAR
        res = isfile_1(&nomfile, cl);
        if (res == NULL) { clnt_perror(cl, "Error en la función isfile"); exit(50); }

        // Si el fichero existe, se continúa, mirando si es legible
        if (res != NULL && res->caso == 0 && res->Resultado_u.b == CIERTO) // A RELLENAR
        {
            fprintf(stderr, "Cliente: %d El fichero %s existe en el servidor.\n", id_cliente, nomfile);

            // A RELLENAR
            res = isreadable_1(&nomfile, cl);
            if (res == NULL) clnt_perror(cl, "Error en la función isreadable");
            ...
            if (res != NULL && res->caso == 0 && res->Resultado_u.b == CIERTO)  // A RELLENAR
            {
                ...
                // A RELLENAR
                res = getnumlines_1(&nomfile, cl);
                if (res == NULL) clnt_perror(cl, "Error en la función isreadable");

                // Si el número de líneas es positivo, se continúa preparándose para descargar
                if (res != NULL && res->caso == 1 && res->Resultado_u.val > 0) // A RELLENAR
                {
                    ...
                    if (numlineas > SLICE)
                    {
                        ...
                        while (linini <= numlineas)
                        {
                            ...
                            q2.lineaini = linini;
                            q2.lineafin = linfin;
                            res = getlines_1(&q2, cl);
                            if (res == NULL) clnt_perror(cl, "Error en la función getlines");
                            ...
                            while (l != NULL)
                            {
                                fprintf(fpout, "%s", l->linea);
                                l = l->sgte;
                            }                        

                            // Se actualizan los límites de la rodaja
                            linini = linfin + 1;  // A RELLENAR
                    ...
                    else // si el número de líneas era menor a SLICE, se descarga línea a línea
                    {
                        ...
                        q1.filename = nomfile;
                            for (i = 1; i <= numlineas; i++)
                            {
                                q1.linea = i;
                                res = getline_1(&q1, cl);
                                if (res == NULL) clnt_perror(cl, "Error en la funcion getline");
                                else if (res->caso == 2) fprintf(fpout, "%s", res->Resultado_u.linea);
                            }
                    }
...
int main(int argc, char *argv[])
{
    ...
    {
        fprintf(stderr, "No se pudo inicializar el mutex para RPC\n");
        exit(6);
    }
    ...
    // A RELLENAR
    if (pthread_mutex_init(&mpet, NULL) != 0)
    {
        fprintf(stderr, "No se pudo inicializar el mutex para el fichero de peticiones\n");
        exit(7);
    }
    ...
    for (i = 0; i < num_clientes; i++)
    {
        ...
        q->id_cliente = i + 1;
        q->fpin = fp;
        if (pthread_create(&th[i], NULL, (void *)procesarDescargas, q) != 0)
        exit(9);
    }
