...
void ejecutar_comando(int sock)
{
    ...
    case QUERY_CMD:
        ...
        for (int i = 0; i < MAX_CLIENTS; i++)
            if (clientes[i] != NULL)
                total++;
        sendShort(sock, total);

        // Enviar todos los endpoints
        for (int i = 0; i < MAX_CLIENTS; i++)
            if (clientes[i] != NULL)
                sendEndpoint(sock, clientes[i]->endpoint);
...
{
    // AÑADIDO: Pelayo Iglesias Manzano - puerto por defecto 11111
    int puerto = 11111;
