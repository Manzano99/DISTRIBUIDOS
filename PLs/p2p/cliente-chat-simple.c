...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        // Llamada a select
        resultado = select(max + 1, &escucha, NULL, NULL, NULL);
        if (resultado == -1)
        {
...
void leer_y_procesar_teclado(int socketUDP)
{
    ...
    if (linea[0] == '/')
    {
        ...
        {
            // Comando /CHAT: Cambiar destino
            char ip[25];
            int puerto;
            if (sscanf(linea, "/CHAT %s %d", ip, &puerto) == 2)
            {
                // Guardar la nueva IP y puerto
                strncpy(ip_destino, ip, sizeof(ip_destino));
                ...
                printf("Nuevo destino establecido: %s:%d\n", ip_destino, puerto_destino);
            }
            else
            {
                printf("Uso incorrecto de /CHAT. Formato correcto: /CHAT <ip> <puerto>\n");
            }
        }
    }
    else      // Si la linea no comienza por /, es un mensaje a enviar al peer
    {
        ...
        mensaje_a_enviar = malloc(strlen(nick) + strlen(linea) + 2);
        sprintf(mensaje_a_enviar, "%s: %s", nick, linea);

        // Enviar el mensaje al destino
        int enviados = sendto(socketUDP, mensaje_a_enviar, strlen(mensaje_a_enviar), 0, 
